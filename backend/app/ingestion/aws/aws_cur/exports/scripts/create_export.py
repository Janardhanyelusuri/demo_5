...
from ..scripts.exports_ops import create_export, update_export, create_boto3_client
from ..scripts.s3 import bucket
import datetime

load_dotenv()


def enable_and_create_export(aws_access_key,
                             aws_secret_key,
                             aws_region,
                             s3_bucket,
                             s3_prefix,
                             export_name):
    # start_date = os.getenv("date")
    ...
    bucket(s3_bucket, aws_region, aws_access_key, aws_secret_key)

    client = create_boto3_client(aws_access_key, aws_secret_key, aws_region)

    try:
        export_arn = create_export(client, export_name, export_description, s3_bucket, s3_prefix, aws_region, query_columns)
    except Exception as ex:
        print(f"error occurred in create_export: {ex}")
        update_export(client, export_name, export_description, s3_bucket, s3_prefix, aws_region, query_columns)
