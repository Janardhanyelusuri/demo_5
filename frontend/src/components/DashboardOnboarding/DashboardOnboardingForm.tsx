...
interface DashboardOnboardingFormProps {
...
const DashboardOnboardingForm: React.FC<DashboardOnboardingFormProps> = ({
  projectName,
  cloud_platforms,
  onBack,
}) => {
  ...
  useEffect(() => {
    const filtered = projects.filter((project) =>
      cloud_platforms.some(
        (platform) => project.cloud_platform === platform.toLowerCase()
      )
    );
    setFilteredProjects(filtered);
  }, [projects, cloud_platforms]);
    ...
    try {
      const response = await axiosInstance.post(
        `${BACKEND}/dashboard/`,
        {
          ...
          cloud_platforms: cloud_platforms.map((p) => p.toLowerCase()),
  ...
  const CustomOption = (props: any) => (
    <components.Option {...props}>
      <input
        ...
        onChange={() => null}
        className="custom-checkbox-margin"
  ...
  return (
    <div className="flex items-center justify-center h-screen">
      <div className="shadow-md border border-[#E0E5EF] rounded-md h-[650px] mb-9 bg-white flex flex-col">
          ...
          <div className="w-[550px]">
            ...
            <Select
              ...
              options={filteredProjects.map((project) => ({
                value: project.id,
                label: `${project.name} (${project.cloud_platform})`,
              }))}
              ...
              placeholder={`Select projects from: ${cloud_platforms.join(", ")}`}
            />
