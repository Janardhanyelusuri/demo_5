...
const GcpCeCostByTime: React.FC<Props> = ({ projectId }) => {
  const [response, setResponse] = useState<any[]>([]);
  const [error, setError] = useState<Error | null>(null);
  const [loading, setLoading] = useState<boolean>(true);
  const cloudPlatform = "gcp";
  const queryType = "gcp_ce_cost_by_time";
  ...
  return (
    <div className="h-full w-full bg-white rounded-lg shadow-md p-4 border flex flex-col">
      <h2 className="mb-4 text-lg">Gcp CE Cost by Time</h2>
      <div className="flex-grow">
        <DynamicChart
          data={response}
          type="line"
          xKey="gcp_fact_dim.billing_period_start"
...
export default GcpCeCostByTime;
