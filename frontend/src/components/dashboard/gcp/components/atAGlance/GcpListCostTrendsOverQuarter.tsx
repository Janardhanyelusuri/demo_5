...
const GcpListCostTrendsOverQuarter: React.FC<Props> = ({ projectId }) => {
  const [response, setResponse] = useState<any[]>([]);
  const [error, setError] = useState<Error | null>(null);
  const [loading, setLoading] = useState<boolean>(true);
  const cloudPlatform = "gcp";
  const queryType = "gcp_list_cost_trends_over_quarter";
  ...
  return (
    <div className="h-full w-full bg-white rounded-lg shadow-md p-4 border flex flex-col">
      <h2 className="mb-4 text-lg">List Cost Trends Over Quarter</h2>
      <div className="flex-grow">
        <DynamicChart
          data={response}
          type="line"
          xKey="gcp_fact_dim.billing_period_start.quarter"
          yKey="gcp_fact_dim.total_list_cost"
          name="List Cost Trends Over Quarter"
...
export default GcpListCostTrendsOverQuarter;
