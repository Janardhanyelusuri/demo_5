...
const AreaChartComponent: React.FC<AreaChartComponentProps> = ({
  ...
  useEffect(() => {
      ...
      while (retries < maxRetries) {
        try {
          const result = await apiQueue.enqueue(
            () => fetchData(cloudPlatform, queryType, projectId),"AreaChart");
    ...
    getData();
  }, [cloudPlatform, queryType, projectId, nameField, valueField, processChartData]);
  ...
  return (
    <div className="h-full w-full bg-white rounded-lg shadow-md p-4 border flex flex-col">
      <div className="flex items-center justify-between mb-4">
        ...
        <AreaChartIcon className="w-6 h-6 text-blue-500" />
      </div>
